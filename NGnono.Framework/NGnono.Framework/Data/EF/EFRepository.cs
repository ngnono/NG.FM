...
namespace NGnono.Framework.Data.EF
{
    ...
    public abstract class EFRepository<T> : IEFRepository<T> where T : BaseEntity
    {
        ...
        public virtual IQueryable<T> Get(Expression<Func<T, bool>> filter, out int total, int index = 0, int size = 50, Func<IQueryable<T>, IOrderedQueryable<T>> orderBy = null, int specialSkipCount = 0)
        {
            ...
            return Get(resetSet, out total, index, size, orderBy);
        }

        /// <summary>
        /// 查询
        /// </summary>
        /// <param name="resetSet">条件</param>
        /// <param name="total">筛选后返回记录数</param>
        /// <param name="index">指定页面索引</param>
        /// <param name="size">指定页面条数</param>
        /// <param name="specialSkipCount"></param>
        /// <param name="orderBy">排序</param>
        /// <returns></returns>
        public virtual IQueryable<T> Get(IQueryable<T> resetSet, out int total, int index = 0, int size = 50, Func<IQueryable<T>, IOrderedQueryable<T>> orderBy = null, int specialSkipCount = 0)
        {
